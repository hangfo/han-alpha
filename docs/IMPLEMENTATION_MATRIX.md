...
| IBKR Paper validation | E1-B PARTIAL REAL / EXTERNAL BLOCKED | TWS Paper 7497, official ibapi 10.48.1, native Keychain account discovery and five eligible empty-account API sessions are VERIFIED; scenario truth checks reject mislabeled state. Static positions, API/ALL orders, resets/recovery, full Golden Tapes and every Broker write remain BLOCKED |
