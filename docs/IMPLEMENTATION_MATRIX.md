...
| IBKR Paper validation | E1-B CODE READY / EXTERNAL BLOCKED | Observer-only structural write denial, dual observation modes, automatic evidence registration, fourteen-scenario Golden Tape transforms and Callback Truth Map are local-test verified; official `ibapi`, authenticated callbacks/resets and every Paper order remain BLOCKED |
| PIT vendor qualification | R1-B CODE READY / EXTERNAL BLOCKED | Bounded redacted probes, immutable raw manifests, typed audits, exact claim binding, expiry and independent Ed25519 review exist; every initial profile remains BLOCKED pending real rights, credentials, samples and review receipts |
| Ops Dashboard | PARTIAL E1 LOCAL | Explicit Heartbeats, current Scope, shared Quote gate, Artifact Registry and Corpus status are source-backed/read-only; deployed alerts/E2E and authenticated write controls are BLOCKED/NOT IMPLEMENTED |
