...
| IBKR Paper validation | E1-B CODE READY / EXTERNAL BLOCKED | Keychain onboarding and resumable API/ALL runner join observer-only structural write denial, evidence registration, fourteen-scenario Golden Tape transforms and Callback Truth Map; official `ibapi`, authenticated callbacks/resets and every Paper order remain BLOCKED |
| PIT vendor qualification | R1-B CODE READY / EXTERNAL BLOCKED | Bounded runner, exact transport/header/normalized evidence, strict portable Artifact authority, typed audits, expiry and independent Ed25519 review exist; every initial profile remains BLOCKED pending real rights, credentials, samples and review receipts |
