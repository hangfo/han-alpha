...
| IBKR Paper validation | E1-B PARTIAL REAL / EXTERNAL BLOCKED | TWS Paper 7497, official ibapi 10.48.1, Keychain account discovery and five eligible empty-account API sessions are VERIFIED. Issue #7 adds per-action fresh Quote rotation, feed-scope fit-for-purpose, zero-cost receipts and legacy lifecycle-ledger migration. The current SPY streaming request still reports no eligible real-time data, so no Permit/write exists and static positions, API/ALL orders, resets/recovery and Golden Tapes remain BLOCKED |
| PIT vendor qualification | R1-B PARTIAL REAL / EXTERNAL BLOCKED | All three credentials are safely present. SEC completed a bounded two-request transport/timing probe but remains blocked on rights and independent review. FRED is policy-blocked before network; Massive is cost/plan-blocked before network. Exact bytes, headers, normalized evidence, stage timings, typed audits and reviewer authority remain fail-closed |
