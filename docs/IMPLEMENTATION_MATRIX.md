...
| IBKR Paper validation | E1-B CODE READY / EXTERNAL BLOCKED | Keychain/stdin-IPC onboarding, safe official API installer, composite Broker identity and resumable API/ALL runner join observer-only structural write denial, evidence registration, fourteen-scenario Golden Tape transforms and Callback Truth Map; license acceptance, authenticated callbacks/resets and every Paper order remain BLOCKED |
