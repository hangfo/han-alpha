...
export function App() {
  ...
  return <main>
    ...
    <section className="grid panels">
      ...
      <article className="panel"><div className="panel-title"><h2>恢复与 Burn-in</h2><span>{data.burn_in.corpus_decision}</span></div><dl><div><dt>当前 Scope</dt><dd>{data.burn_in.scope_hash ? shortId(data.burn_in.scope_hash) : "无 Scope"}</dd></div><div><dt>Completed Orders</dt><dd>{data.burn_in.scope_policy?.completed_orders_api_only === true ? "API ONLY" : data.burn_in.scope_policy?.completed_orders_api_only === false ? "API + MANUAL" : "未验证"}</dd></div><div><dt>备份年龄</dt><dd>{secondsLabel(data.backup.age_seconds)}</dd></div><div><dt>完整观察</dt><dd>{data.burn_in.completed_observation_sessions} / {data.burn_in.target_sessions}</dd></div><div><dt>连续稳定</dt><dd>{data.burn_in.consecutive_stable_sessions} / {data.burn_in.target_sessions}</dd></div><div><dt>进程重启</dt><dd>{data.burn_in.process_restarts} / {data.burn_in.target_process_restarts}</dd></div><div><dt>TWS 重启</dt><dd>{data.burn_in.tws_restarts} / {data.burn_in.target_tws_restarts}</dd></div><div><dt>Nightly reset</dt><dd>{data.burn_in.nightly_resets} / {data.burn_in.target_nightly_resets}</dd></div><div><dt>Golden Tape</dt><dd>{data.burn_in.golden_tapes} / {data.burn_in.target_golden_tapes} · {data.burn_in.golden_tape_decision}</dd></div></dl></article>
      ...
      <article className="panel wide"><div className="panel-title"><h2>Evidence Registry</h2><span>{data.evidence_registry.verified} / {data.evidence_registry.total} verified</span></div><div className="rows">{data.evidence_registry.recent.length === 0 ? <p className="empty">尚无注册证据</p> : data.evidence_registry.recent.slice(0, 10).map(item => <div className="row" key={item.artifact_id}><span>{item.artifact_type} · {shortId(item.artifact_id)}</span><b className={item.verified ? "ok-text" : "bad-text"}>{item.verified ? "VERIFIED" : item.status}</b></div>)}</div></article>
      <article className="panel wide"><div className="panel-title"><h2>外部验收队列</h2><span>只统计可解析的 Registry 证据</span></div><div className="rows"><div className="row"><span>E1 API Scope</span><b>{data.external_acceptance.e1.api.completed} / {data.external_acceptance.e1.api.required} · {data.external_acceptance.e1.api.decision}</b></div><div className="row"><span>E1 ALL Scope</span><b>{data.external_acceptance.e1.all.completed} / {data.external_acceptance.e1.all.required} · {data.external_acceptance.e1.all.decision}</b></div>{Object.entries(data.external_acceptance.r1).map(([source, item]) => <div className="row" key={source}><span>R1 {source}</span><b>{item.sample_manifests} samples · {item.decision}</b></div>)}</div></article>
