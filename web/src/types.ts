...
export type OpsOverview = {
  ...
  freshness: { api_age_seconds: number; authority_age_seconds: number | null; observer_fact_age_seconds: number | null; quote_age_seconds: number | null; reconciliation_age_seconds: number | null };
  readiness: Record<"service" | "observer" | "authority" | "shadow" | "paper_canary", ReadinessLayer>;
  ...
  burn_in: { independent_sessions: number; target_sessions: number; process_restarts: number; target_process_restarts: number; tws_restarts: number; target_tws_restarts: number; nightly_resets: number; target_nightly_resets: number; golden_tapes: number; target_golden_tapes: number };
